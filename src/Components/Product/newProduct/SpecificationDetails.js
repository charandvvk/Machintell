...
function SpecificationDetails({type}) {
  ...
  const specifications =
    type === "product"
      ...
      const [specificationsState, setSpecificationsState] = useState(specifications);
  const handleInputChange = (value, rowIndex, cellIndex) => {
    setSpecificationsState((prevState) => {
      const updatedSpecificationsState = prevState.map((specification) => [
        ...specification,
      ]);
      ...
      if(type==="product"){
          ...
          dispatch(
            productActions.addProductSpecifications(
              specificationsState.map((specification) => [...specification])
            )
          );
      }
      else{
  ...
  return (
    <div>
      ...
      <div>
        ...
        <button className={styles.btn} onClick={handleDeleteRow}>
          -
        </button>
